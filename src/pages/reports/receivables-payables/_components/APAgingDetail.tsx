 ...
 * Every unpaid bill: vendor, bill title, date, due date, days overdue,
 * amount, paid, balance, ageing bucket.
 * Total reconciles to AP Ageing Summary and AP control account.
 */
...
export default function APAgingDetail() {
  ...
  const vendors = useQuery(api.reports.receivablesPayables.listVendors);
  const data = useQuery(api.reports.receivablesPayables.getAPAgingDetail, {
    asOfDate: dateRange.endDate,
    ...(vendorId !== "all" ? { vendorId: vendorId as Id<"vendors"> } : {}),
    ...(bucket !== "all" ? { bucket: bucket as "current" | "days30" | "days60" | "days90" | "over90" } : {}),
  });

  const handleExport = () => {
    if (!data) return;
    const header = "Vendor,Bill,Due Date,Days Past,Original,Paid,Balance,Bucket\n";
    const rows = data.rows.map((r) =>
      `"${r.vendorName}","${r.billTitle}","${r.dueDate}",${r.daysPast},${r.originalAmount},${r.amountPaid},${r.balance},"${r.bucket}"`
    ).join("\n");
    const blob = new Blob([header + rows], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `ap-aging-detail-${data.asOfDate}.csv`; a.click();
  ...
  const toolbar = (
    <div className="flex flex-wrap gap-2">
      <Select value={vendorId} onValueChange={setVendorId}>
        <SelectTrigger className="w-44 h-8 text-xs cursor-pointer"><SelectValue placeholder="All Vendors" /></SelectTrigger>
        <SelectContent>
  ...
  return (
    <ReportShell
      ...
      subtitle={data ? `As of ${data.asOfDate} — ${data.rows.length} unpaid bills` : undefined}
      dateRange={dateRange}
      ...
      {!data ? (
        ...
        <>
          <div className={cn(
            ...
            <span>
              AP Control Account <strong>{fmt(data.controlBalance)}</strong>
          ...
          <div className="overflow-x-auto rounded-lg border">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {data.rows.length === 0 ? (
                  <tr><td colSpan={8} className="p-6 text-center text-muted-foreground">No unpaid bills found.</td></tr>
                ) : data.rows.map((row) => (
              ...
              {data.rows.length > 0 && (
                <tfoot className="bg-muted/50 font-bold border-t">
                  <tr>
                    <td colSpan={4} className="p-2 text-right text-xs uppercase tracking-wide text-muted-foreground">Totals</td>
